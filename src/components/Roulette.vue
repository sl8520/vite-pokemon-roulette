...
export default defineComponent({
  ...
  methods: {
    reset() {
      ...
      document.querySelector(`#wheel-container-${this.randomIdRoulette} .wheel`).style.transform = `rotate(${this.startingAngle}deg)`
    },
    launchWheel() {
      ...
      // 2. 計算目標 item 的角度
      const targetAngle = this.counterClockWiseOperator * (
        -(wheelResult) * this.itemAngle - this.itemAngle / 2 + this.degreesVariation
      )
      ...
      // 4. 計算這次要轉到的最終角度
      const currentMod = this.currentRotation % fullCircle
      let delta = targetAngle - currentMod
      // 保證 delta 是正的（順時針），如果你是逆時針，根據需求調整
      if (delta < 0) delta += fullCircle

      const rotateTo = this.currentRotation + extraRounds * fullCircle + delta

      // 5. 設定 transform
      wheelElt.style.transform = `rotate(${rotateTo}deg)`
